...
void mutex_aquire(mutex_t *mut) {
  ...
  do {
    ...
    else // even if this is our pid, we do not care, correct usage is not to try
         // to aquire the mutex after you already have it
    {
...
void mutex_release(mutex_t *mut) {
  ...
  // steps: if task in delayed list,pop a task from the delayed list and add
  // back to ready list
